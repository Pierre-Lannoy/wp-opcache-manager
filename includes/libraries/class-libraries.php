...
namespace OPcacheManager\Library;
...
class Libraries {
	...
	public static function init() {
		...
		self::$psr4_libraries['markdown'] = [
			...
			'base'    => OPCM_VENDOR_DIR . 'markdown/',
			'version' => '1.2.1',
			// phpcs:ignore
			'author'  => sprintf( esc_html__( '%s & contributors', 'opcache-manager' ), 'Carsten Brandt' ),
		...
		self::$psr4_libraries['psr-03']      = [
			'name'    => 'PSR-3',
			'prefix'  => 'Psr\\Log',
			'base'    => OPCM_VENDOR_DIR . 'psr/log-v' . $psrlog_version . '/',
		...
		self::$psr4_libraries['psr-07']      = [
			...
			'base'    => OPCM_VENDOR_DIR . 'psr/http-message/',
			'version' => '2.0',
			'author'  => 'PHP Framework Interop Group',
			'url'     => 'https://www.php-fig.org/',
			'license' => 'mit',
		...
		self::$psr4_libraries['psr-18']      = [
			...
			'base'    => OPCM_VENDOR_DIR . 'psr/http-client/',
			'version' => '1.0.3',
			'author'  => 'PHP Framework Interop Group',
			'url'     => 'https://www.php-fig.org/',
			'license' => 'mit',
		...
		self::$psr4_libraries['decalog-sdk'] = [
			'name'    => 'DecaLog SDK',
			'prefix'  => 'DecaLog',
			'base'    => OPCM_VENDOR_DIR . 'decalog-sdk/',
			'version' => '4.2.0',
	...
	private function license_name( $license ) {
		switch ( $license ) {
			case 'mit':
				$result = esc_html__( 'MIT license', 'opcache-manager' );
				break;
			case 'apl2':
				$result = esc_html__( 'Apache license, version 2.0', 'opcache-manager' );
				break;
			case 'gpl2':
				$result = esc_html__( 'GPL-2.0 license', 'opcache-manager' );
				break;
			case 'gpl3':
				$result = esc_html__( 'GPL-3.0 license', 'opcache-manager' );
				break;
			default:
				$result = esc_html__( 'unknown license', 'opcache-manager' );
				break;
		}
		return $result;
	}
	...
	public function sc_get_list( $attributes ) {
		...
		if ( 'html' === $style ) {
			...
			foreach ( $list as $library ) {
				...
				$items[] = sprintf( __( '<a href="%1$s">%2$s %3$s</a>%4$s by %5$s (%6$s)', 'opcache-manager' ), $library['url'], $library['name'], 'v' . $library['version'], $library['langs'], $library['author'], $library['license'] );
			}
