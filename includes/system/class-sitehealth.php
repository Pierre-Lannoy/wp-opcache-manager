...
class Sitehealth {
	...
	public function perfopsone_info( $debug_info ) {
		...
		if ( ! array_key_exists( $key, $debug_info ) ) {
			$debug_info[ $key ] = [
				'label'  => esc_html__( 'Object cache', 'opcache-manager' ),
	...
	public function perfopsone_test_objectcache_do() {
		...
		if ( 'db_transient' === $analytics['type'] ) {
			$result = [
				...
				'description' => sprintf( '<p>%s %s</p>', esc_html__( 'Your site uses database transient.', 'opcache-manager' ), esc_html__( 'You should consider using a dedicated object caching mechanism, like Memcached or Redis, to improve your site\'s speed.', 'opcache-manager' ) ),
				'actions'     => '',
				'test'        => $key,
			];
			...
			$result = [
				...
				'description' => sprintf( '<p>%s %s</p>', esc_html__( 'Your site uses APCu, but only a few plugins know how to take advantage of it.', 'opcache-manager' ), esc_html__( 'You should consider using a dedicated object caching mechanism, like Memcached or Redis, to improve your site\'s speed.', 'opcache-manager' ) ),
				'actions'     => '',
				'test'        => $key,
			];
