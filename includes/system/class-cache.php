...
class Cache {
	 ...
	 */
	private static $pool_name = OPCM_SLUG;
		...
		if ( self::$apcu_available ) {
			if ( function_exists( 'apcu_cache_info' ) && function_exists( 'apcu_delete' ) ) {
				try {
					...
					\DecaLog\Engine::eventsLogger( OPCM_SLUG )->error( sprintf( 'Unable to query APCu status: %s.', $e->getMessage() ), [ 'code' => $e->getCode() ] );
				}
			}
		} else {
	...
	public static function log_debug() {
		...
		if ( 0 !== (int) $analytics['hit']['count'] || 0 !== (int) $analytics['miss']['count'] ) {
			\DecaLog\Engine::eventsLogger( OPCM_SLUG )->debug( $log );
		}
	}
