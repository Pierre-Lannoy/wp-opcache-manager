...
class Favicon {
	...
	public static function get_raw( $name = 'wordpress.org', $force_download = false ) {
		...
		if ( ! file_exists( $dir ) ) {
			try {
				mkdir( $dir, 0755, true );
				\DecaLog\Engine::eventsLogger( OPCM_SLUG )->info( 'Created: "' . $dir . '" favicons cache directory.' );
			} catch ( \Exception $ex ) {
				\DecaLog\Engine::eventsLogger( OPCM_SLUG )->error( 'Unable to create "' . $dir . '" favicons cache directory.' );
				return self::get_default();
			}
		}
		if ( ! file_exists( $filename ) ) {
			...
			if ( is_wp_error( $response ) ) {
				\DecaLog\Engine::eventsLogger( OPCM_SLUG )->error( 'Unable to download "' . $name . '" favicon: ' . $response->get_error_message(), [ 'code' => $response->get_error_code() ] );
				return self::get_default();
			}
			...
			if ( 200 !== (int) wp_remote_retrieve_response_code( $response ) && 404 !== (int) wp_remote_retrieve_response_code( $response ) ) {
				\DecaLog\Engine::eventsLogger( OPCM_SLUG )->error( 'Unable to download "' . $name . '" favicon.', [ 'code' => wp_remote_retrieve_response_code( $response ) ] );
				return self::get_default();
			}
			...
			if ( $wp_filesystem->errors->has_errors() ) {
				foreach ( $wp_filesystem->errors->get_error_messages() as $message ) {
					\DecaLog\Engine::eventsLogger( OPCM_SLUG )->error( 'Unable to download "' . $name . '" favicon: ' . $message );
				}
				return self::get_default();
			}
			\DecaLog\Engine::eventsLogger( OPCM_SLUG )->debug( 'Favicon downloaded for "' . $name . '".' );
		}
		// phpcs:ignore
