	...
	class Menus {
		...
		public static function finalize() {
			...
			if ( ! self::$initialized ) {
				...
				add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Control Center', 'opcache-manager' ), __( 'Control Center', 'opcache-manager' ), 'manage_options', 'perfopsone-dashboard', [ self::class, 'get_dashboard_page' ] );
			}
				...
				if ( ! self::$initialized ) {
					switch ( $menu ) {
						...
						case 'insights':
							...
							break;
						case 'records':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Catalogs', 'opcache-manager' ), __( 'Catalogs', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ] );
							break;
						case 'consoles':
			...
			if ( array_key_exists( 'consoles', self::$menus ) ) {
				...
				foreach ( self::$menus['consoles'] as $item ) {
					...
					if ( $item['activated'] ) {
						...
						$i['text'] = esc_html__( 'This console is currently unavailable. Click here to see why.', 'opcache-manager' );
						$i['url']  = $item['remedy'];
					}
					$items[] = $i;
				}
