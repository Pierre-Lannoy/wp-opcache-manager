	...
	class Menus {
		...
		public static function finalize() {
			if ( ! self::$initialized ) {
				...
				add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Control Center', 'opcache-manager' ), __( 'Control Center', 'opcache-manager' ), 'manage_options', 'perfopsone-dashboard', [ self::class, 'get_settings_page' ] );
			}
			...
			foreach ( self::$menus_positions as $menu ) {
				if ( ! self::$initialized ) {
					switch ( $menu ) {
						case 'analytics':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Analytics', 'opcache-manager' ), __( 'Analytics', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ] );
							break;
						case 'tools':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Tools', 'opcache-manager' ), __( 'Tools', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ] );
							break;
						case 'insights':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Reports', 'opcache-manager' ), __( 'Reports', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ] );
							break;
						case 'records':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Catalogues', 'opcache-manager' ), __( 'Catalogues', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ] );
							break;
						case 'consoles':
							add_submenu_page( 'perfopsone-dashboard', esc_html__( 'Consoles', 'opcache-manager' ), __( 'Consoles', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ] );
							break;
					}
				}
			...
			if ( array_key_exists( 'analytics', self::$menus ) ) {
				...
				foreach ( self::$menus['analytics'] as $item ) {
					...
					if ( $item['activated'] ) {
						...
						$i['text'] = esc_html__( 'This analytics feature is currently disabled. Click here to activate it.', 'opcache-manager' );
						$i['url']  = $item['remedy'];
					}
					$items[] = $i;
				}
		...
		public static function get_settings_page() {
			if ( array_key_exists( 'settings', self::$menus ) ) {
				...
				foreach ( self::$menus['settings'] as $item ) {
					...
					if ( $d->waiting_update() ) {
						$i['need_update'] = sprintf( esc_html__( 'Need to be updated to %s.', 'opcache-manager' ), $d->waiting_update() );
					} else {
						$i['need_update'] = '';
					}
					if ( $d->is_required_wp_ok() ) {
						$i['need_wp_update'] = '';
						$i['ok_wp_update']   = esc_html__( 'OK', 'opcache-manager' );
					} else {
						$i['need_wp_update'] = esc_html__( 'need update', 'opcache-manager' );
						$i['ok_wp_update']   = '';
					}
					if ( $d->is_required_php_ok() ) {
						$i['need_php_update'] = '';
						$i['ok_php_update']   = esc_html__( 'OK', 'opcache-manager' );
					} else {
						$i['need_php_update'] = esc_html__( 'need update', 'opcache-manager' );
						$i['ok_php_update']   = '';
					}
					$i['icon'] = call_user_func( $item['icon_callback'] );
					$i['slug'] = $item['plugin'];
					...
					if ( 0 < (int) $i['installs'] ) {
						$i['installs'] = sprintf( esc_html__( '%s+ installs.', 'opcache-manager' ), Conversion::number_shorten( (int) $i['installs'], 0 ) );
					} else {
						$i['installs'] = '';
					}
					if ( 0 < (int) $i['downloads'] ) {
						$i['downloads'] = sprintf( esc_html__( '%s downloads.', 'opcache-manager' ), Conversion::number_shorten( (int) $i['downloads'], 2 ) );
					} else {
						$i['downloads'] = '';
					}
					if ( 0 < (int) $i['reviews'] ) {
						$i['reviews'] = sprintf( esc_html__( '%s reviews.', 'opcache-manager' ), Conversion::number_shorten( (int) $i['reviews'], 0 ) );
						$i['rating']  = $i['rating'] / 20;
					} else {
						$i['reviews'] = esc_html__( 'No review yet.', 'opcache-manager' );
						$i['rating']  = 0.0;
					}
					$i['stars'] = '';
					if ( 0 < (int) $i['rating'] ) {
		...
		private static function display_as_controls( $items ) {
			...
			$disp       .= '   } );';
			$disp       .= '  </script>';
			$main_update = '<span class="poo-noneed-update">' . esc_html__( 'Up to date', 'opcache-manager' ) . '</span>';
			foreach ( $items as $item ) {
				if ( $item['need_update'] ) {
					$update      = '<a href=" ' .admin_url( 'update-core.php' ) . '"><span class="poo-need-update">' . esc_html__( 'Need update', 'opcache-manager' ) . '</span></a>';
					$main_update = $update;
				} else {
					$update = '<span class="poo-noneed-update">' . esc_html__( 'Up to date', 'opcache-manager' ) . '</span>';
				}
				if ( ! current_user_can( 'update_plugins' ) || ! wp_is_auto_update_enabled_for_type( 'plugin' ) || ( is_multisite() && ! is_network_admin() ) ) {
					$auto = '';
				} else {
					$auto  = '<div class="poo-switch" data-value="' . $item['slug'] . '">' . esc_html__( 'auto-update', 'opcache-manager' ) . ' &nbsp;';
				...
				$support  = '<a title="' . esc_html__( 'Get support', 'opcache-manager' ) . '" alt="' . esc_html__( 'Get support', 'opcache-manager' ) . '" class="poo-links-support" href="https://wordpress.org/support/plugin/' . $item['slug'] . '" target="_blank"><img style="width:16px;padding:0 4px;" src="' . \Feather\Icons::get_base64( 'message-circle', 'none', '#73879C', 3 ) . '" /></a>';
				$contrib  = '<a title="' . esc_html__( 'Contribute', 'opcache-manager' ) . '" alt="' . esc_html__( 'Contribute', 'opcache-manager' ) . '" class="poo-links-contrib" href="https://github.com/Pierre-Lannoy/wp-' . $item['slug'] . '" target="_blank"><img style="width:16px;padding:0 4px;" src="' . \Feather\Icons::get_base64( 'github', 'none', '#73879C', 3 ) . '" /></a>';
				$star     = '<a title="' . esc_html__( 'Make a review', 'opcache-manager' ) . '" alt="' . esc_html__( 'Make a review', 'opcache-manager' ) . '" class="poo-links-star" href="https://wordpress.org/support/plugin/' . $item['slug'] . '/reviews" target="_blank"><img style="width:16px;padding:0 4px;" src="' . \Feather\Icons::get_base64( 'star', 'none', '#73879C', 3 ) . '" /></a>';
				$settings = '<a class="poo-action" href="' . $item['url'] . '"/><img style="width:18px;padding:0 4px;" src="' . \Feather\Icons::get_base64( 'settings', 'none', '#73879C', 3 ) . '" />' . esc_html__( 'settings', 'opcache-manager' ) . '</a>';
				$about    = '<a class="poo-action" href="' . $item['url'] . '&tab=about"/><img style="width:18px;padding:0 4px;" src="' . \Feather\Icons::get_base64( 'info', 'none', '#73879C', 3 ) . '" />' . esc_html__( 'about', 'opcache-manager' ) . '</a>';
			...
			$site  = '<a class="poo-action" href="https://perfops.one" target="_blank"><img style="width:18px;padding:0 4px;vertical-align: bottom !important;" src="' . \Feather\Icons::get_base64( 'home', 'none', '#73879C', 3 ) . '" />' . esc_html__( 'all plugins', 'opcache-manager' ) . '</a>';
			$disp .= '<div class="poo-container">';
			$disp .= ' <div class="poo-actionable">';
