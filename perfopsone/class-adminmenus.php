...
if ( ! class_exists( 'PerfOpsOne\AdminMenus' ) ) {
	class AdminMenus {
		...
		public static function initialize() {
			foreach ( apply_filters( 'init_perfops_admin_menus', [] ) as $menu => $submenus ) {
				if ( ! in_array( 'perfopsone-' . $menu, self::$slugs, true ) ) {
					switch ( $menu ) {
						case 'analytics':
							add_menu_page( esc_html__( 'Dashboard', 'opcache-manager' ), sprintf( esc_html__( '%s Analytics', 'opcache-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 'dashicons-chart-bar', 210874 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Dashboard', 'opcache-manager' ), __( 'Dashboard', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 0 );
							break;
						case 'tools':
							add_menu_page( esc_html__( 'Available Tools', 'opcache-manager' ), sprintf( esc_html__( '%s Tools', 'opcache-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 'dashicons-admin-tools', 210876 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Tools', 'opcache-manager' ), __( 'Available Tools', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 0 );
							break;
						case 'insights':
							add_menu_page( esc_html__( 'Available Reports', 'opcache-manager' ), sprintf( esc_html__( '%s Insights', 'opcache-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 'dashicons-lightbulb', 210875 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Reports', 'opcache-manager' ), __( 'Available Reports', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 0 );
							break;
						case 'records':
							add_menu_page( esc_html__( 'Available Catalogues', 'opcache-manager' ), sprintf( esc_html__( '%s Records', 'opcache-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 'dashicons-book', 210873 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Catalogues', 'opcache-manager' ), __( 'Available Catalogues', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 0 );
							break;
						case 'settings':
							add_menu_page( esc_html__( 'Control Center', 'opcache-manager' ), sprintf( esc_html__( '%s Settings', 'opcache-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 'dashicons-admin-settings', 210871 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Control Center', 'opcache-manager' ), __( 'Control Center', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 0 );
							break;
						case 'consoles':
							add_menu_page( esc_html__( 'Available Consoles', 'opcache-manager' ), sprintf( esc_html__( '%s Consoles', 'opcache-manager' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 'dashicons-embed-generic', 210872 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Consoles', 'opcache-manager' ), __( 'Available Consoles', 'opcache-manager' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 0 );
							break;
					}
		...
		private static function display_as_lines( $items ) {
			...
			foreach ( $items as $item ) {
				$links   = [];
				$links[] = '<a href="' . $item['url'] . '"/>' . __( 'settings', 'opcache-manager' ) . '</a>';
				$links[] = '<a href="' . $item['url'] . '&tab=about"/>' . __( 'about', 'opcache-manager' ) . '</a>';
				$links[] = '<a href="https://wordpress.org/support/plugin/' . $item['slug'] . '" target="_blank">' . __( 'support', 'opcache-manager' ) . '</a>';
				$links[] = '<a href="https://github.com/Pierre-Lannoy/wp-' . $item['slug'] . '" target="_blank">' . __( 'contribution', 'opcache-manager' ) . '</a>';
				$disp   .= '<div class="po-container">';
				$disp   .= ' <div class="po-actionable">';
